...
#include <chrono>
#include <random>
#include "SortAlgorithmDll.h"
using namespace std;

// ==================== 1. 冒泡排序 ====================
// 时间复杂度：O(n²)  空间复杂度：O(1)  稳定
void algorithmDynamic::BubbleSort(std::vector<int>& arr) {
    int n = arr.size();
    for (int i = 0; i < n - 1; i++) {
        bool swapped = false;
...
void algorithmDynamic::SelectionSort(std::vector<int>& arr) {
    int n = arr.size();
    for (int i = 0; i < n - 1; i++) {
        int minIdx = i;
        for (int j = i + 1; j < n; j++) {
...
// ==================== 3. 插入排序 ====================
// 时间复杂度：O(n²)  空间复杂度：O(1)  稳定
void algorithmDynamic::InsertionSort(vector<int>& arr) {
    int n = arr.size();
    for (int i = 1; i < n; i++) {
        int key = arr[i];
        int j = i - 1;
...
void algorithmDynamic::ShellSort(vector<int>& arr) {
    int n = arr.size();
...
void algorithmDynamic::MergeSort(vector<int>& arr) {
...
void algorithmDynamic::QuickSort(vector<int>& arr) {
...
void algorithmDynamic::HeapSort(vector<int>& arr) {
    int n = arr.size();
